	...
	while (read(fd, &c, 1) == 1) {
		...
		case '\0':
			if (i == 2048 || i == 4097 || i == 4098) {
				dprintf("Error: missplaced \\0");
				exit(EXIT_FAILURE);
			}
			break;
		case '1':
			if (i != 4097) {
				dprintf("Error: missplaced 1");
				exit(EXIT_FAILURE);
			}
			break;
		case '2':
			if (i != 2048) {
				dprintf("Error: missplaced 2");
				exit(EXIT_FAILURE);
			}
			break;
		case EOF:
			if (i != 4098) {
				dprintf("Error: missplaced EOF");
				exit(EXIT_FAILURE);
			}
			break;
		default:
			dprintf("Error: unexpected character");
			exit(EXIT_FAILURE);
		}
	}
	struct timespec end = get_time();
